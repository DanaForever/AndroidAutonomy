...
def pil_image_to_base64(image):
    """Encode a PIL image to base64 with smart_resize."""
    buffer = BytesIO()
...
def pil_to_base64(screenshot_path):
    ...
    return base64_img, image, ori_width, ori_height

def query_model(client, model_name, instruction, base64_img):
    """Send a single grounding query to the model and return parsed coordinates."""
...
def process_case(case, image_root, output_file, client, model_name, zoom_in=False):
    try:
        image_path = os.path.join(image_root, case['img_filename'])
        base64_img, pil_image, ori_width, ori_height = pil_to_base64(image_path)
        ...
        # First pass
        response_content = query_model(client, model_name, case['instruction'], base64_img)
        ...
        # Zoom-in: second pass with cropped region centered on coarse prediction
        if zoom_in and norm_x is not None:
            crop_w = ori_width / 2
            crop_h = ori_height / 2
            # Center crop on the coarse prediction, clamp to image bounds
            crop_x1 = max(0, min(abs_x - crop_w / 2, ori_width - crop_w))
            crop_y1 = max(0, min(abs_y - crop_h / 2, ori_height - crop_h))
            crop_x2 = crop_x1 + crop_w
            crop_y2 = crop_y1 + crop_h

            cropped = pil_image.crop((crop_x1, crop_y1, crop_x2, crop_y2))
            # Resize crop back to original resolution
            cropped = cropped.resize((ori_width, ori_height), Image.LANCZOS)
            zoom_base64 = pil_image_to_base64(cropped)

            zoom_response = query_model(client, model_name, case['instruction'], zoom_base64)
            zoom_x, zoom_y = parse_coordinates(zoom_response)

            if zoom_x != -1 and zoom_y != -1:
                # Map refined coordinates back to original image space
                refined_norm_x = zoom_x / 1000.0
                refined_norm_y = zoom_y / 1000.0
                abs_x = crop_x1 + refined_norm_x * crop_w
                abs_y = crop_y1 + refined_norm_y * crop_h
                norm_x = abs_x / ori_width
                norm_y = abs_y / ori_height
                response_content = zoom_response
        ...
        gt_cx = (bbox[0] + bbox[2]) / 2
        gt_cy = (bbox[1] + bbox[3]) / 2
        gt_norm_x = gt_cx / img_width
        gt_norm_y = gt_cy / img_height

        print(f"[{case.get('id', case.get('img_filename'))}] "
              f"GT  norm=({gt_norm_x:.4f}, {gt_norm_y:.4f}) abs=({gt_cx:.1f}, {gt_cy:.1f}) | "
              f"Pred norm=({norm_x:.4f}, {norm_y:.4f}) abs=({abs_x:.1f}, {abs_y:.1f}) | "
              f"{result['correctness']}" if norm_x is not None else
              f"[{case.get('id', case.get('img_filename'))}] "
              f"GT  norm=({gt_norm_x:.4f}, {gt_norm_y:.4f}) abs=({gt_cx:.1f}, {gt_cy:.1f}) | "
              f"Pred: {result['correctness']}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--zoom_in", action="store_true", help="Enable zoom-in strategy (two-pass inference)")
