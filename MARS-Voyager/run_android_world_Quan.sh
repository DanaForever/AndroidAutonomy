...
EMULATOR_PATH="${EMULATOR_PATH:-/data/Android/Sdk/emulator/emulator}"
ADB_PATH="${ADB_PATH:-/data/Android/Sdk/platform-tools/adb}"
export ANDROID_SDK_ROOT="${ANDROID_SDK_ROOT:-/data/Android/Sdk}"
export ANDROID_AVD_HOME="${ANDROID_AVD_HOME:-/data/Android/.android/avd}"
...
create_avd_copy() {
    ...
    local source_dir
    source_dir=$(grep "^path=" "$source_ini" | cut -d= -f2)
...
wait_for_emulator_ready() {
    ...
    local deadline=$((SECONDS + EMU_WAIT_TIME + 120))

    log "  [WAIT] Worker $worker_id: waiting for $serial to come online"

    while [[ $SECONDS -lt $deadline ]]; do
        timeout 10 "$ADB_PATH" -P "$adb_server_port" -s "$serial" wait-for-device >/dev/null 2>&1 || true

        local state
        state=$(timeout 5 "$ADB_PATH" -P "$adb_server_port" -s "$serial" get-state 2>/dev/null | tr -d '\r[:space:]')
        ...
        boot_completed=$(timeout 5 "$ADB_PATH" -P "$adb_server_port" -s "$serial" shell getprop sys.boot_completed 2>/dev/null | tr -d '\r[:space:]')
        ...
        local current_user
        current_user=$(timeout 5 "$ADB_PATH" -P "$adb_server_port" -s "$serial" shell whoami 2>/dev/null | tr -d '\r[:space:]')
        if [[ "$current_user" != "root" ]]; then
            timeout 10 "$ADB_PATH" -P "$adb_server_port" -s "$serial" root >/dev/null 2>&1 || true
            sleep 3
            continue
        fi

        log "  [READY] Worker $worker_id: $serial booted and adb is root"
        return 0
    done

    log "  [WARN] Worker $worker_id: $serial did not become ready before timeout"
    return 1
}
...
wait_emulators() {
    ...
    log "Checking emulator readiness on each ADB server..."
    local ready_workers=0
    for i in $(seq 0 $((NUM_WORKERS - 1))); do
        local console_port=$((START_PORT + i * 2))
        local adb_server_port=$((ADB_SERVER_START_PORT + i))
        wait_for_emulator_ready "$i" "$console_port" "$adb_server_port" && ready_workers=$((ready_workers + 1))
    done

    [[ $ready_workers -eq 0 ]] && { log "Error: No emulators became ready"; return 1; }
    ALIVE_WORKERS=$ready_workers
    log "Total ready emulators: $ready_workers"
    return 0
}
...
[[ ! -f "$ANDROID_AVD_HOME/$AVD_NAME.ini" ]] && die "AVD not found: $AVD_NAME (looked for $ANDROID_AVD_HOME/$AVD_NAME.ini)"
