...
CONFIG_NAME="${CONFIG_NAME:-MAI-UI}"
MODEL_NAME="${MODEL_NAME:-MAI-UI}"
...
cleanup() {
    ...
    pkill -f "python.*test_android_world_maiui.py" 2>/dev/null || true
    sleep 2
    pkill -9 -f "python.*test_android_world_maiui.py" 2>/dev/null || true
...
run_eval() {
    ...
    python test_android_world_maiui.py \
        --config "$CONFIG_FILE" \
        --num_workers "$workers" \
