...
CONFIG_NAME="${CONFIG_NAME:-UI-Venus}"
MODEL_NAME="${MODEL_NAME:-UI-Venus}"
...
EMU_MEMORY=8192
EMU_CORES=4
EMU_WAIT_TIME=45 # can be increased to 90
...
cleanup() {
    ...
    pkill -f "python.*test_android_world_uivenus.py" 2>/dev/null || true
    sleep 2
    pkill -9 -f "python.*test_android_world_uivenus.py" 2>/dev/null || true
...
run_eval() {
    ...
    python test_android_world_uivenus.py \
        --config "$CONFIG_FILE" \
        --num_workers "$workers" \
