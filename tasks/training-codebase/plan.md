...
2. **Build a small SFT dataset.** See sub-plan: [`sft-data/plan.md`](./sft-data/plan.md). Sources:
   - Convert successful trajectories from [`MARS-Voyager/eval_results/`](../../MARS-Voyager/eval_results/) into `(screenshot, history, goal) → action` SFT pairs using the existing [`qwen_agent.py`](../../MARS-Voyager/androidworld/eval/agents/qwen_agent.py) prompt format. ([`Qwen3VLAgent`](../../MARS-Voyager/androidworld/eval/agents/qwen_agent.py) already has optional SFT data collection per [`MARS-Voyager/CLAUDE.md`](../../MARS-Voyager/CLAUDE.md) — check that path first.)
   - Optionally mix in a public GUI dataset (AITZ, AndroidControl, GUIAct) for breadth.
