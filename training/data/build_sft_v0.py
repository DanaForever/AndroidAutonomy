"""Build SFT dataset v0 from MARS-Voyager UI-Voyager rollouts.

See `tasks/training-codebase/sft-data/plan.md` for the design rationale.
...
Pipeline:
    ...
    5. Shuffle with a fixed seed; write `train.parquet`.
    6. Best-effort smoke test through verl's MultiturnSFTDataset.
    7. Write manifest.json, README.md, stats/stats.md, stats/plots/*.png.

Hard deps: stdlib, pyyaml, PIL, pyarrow.
Soft deps: matplotlib (plots), transformers (token-length stats), verl + torch
(smoke test). Missing soft deps log a warning and the corresponding artifact
is skipped or downgraded; the build still succeeds.

Usage:
    python3 training/data/build_sft_v0.py \
        --eval-results MARS-Voyager/eval_results \
        --out-dir training/data/sft_v0 \
...
log = logging.getLogger("build_sft_v0")
...
def seed_key_for(run: RunInfo, repeat_id: int) -> str:
    """Dedupe key encoding the seed. Null seed → unique per (run, repeat)."""
    seed = (run.config.get("eval") or {}).get("task_random_seed")
    if seed is None:
        return f"random_{run.run_id}_repeat{repeat_id}"
    return f"seed_{seed}"
...
# ---------------------------------------------------------------------------
_REPEAT_RE = re.compile(r"^repeat_(\d+)_succ\.jsonl$")


def iter_succ_rollouts(run: RunInfo) -> Iterator[tuple[str, int, Path]]:
    """Yield (task_name, repeat_id, rollout_path) for every *_succ.jsonl."""
    for task_dir in sorted(run.run_path.iterdir()):
        ...
        for jsonl in sorted(task_dir.glob("repeat_*_succ.jsonl")):
            ...
            if not m:
                continue
            yield task_dir.name, int(m.group(1)), jsonl
...
def select_one_per_group(
    candidates: list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]],
) -> list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]]:
    ...
    """
    groups: dict[tuple[str, str], list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]]] = {}
    for run, task, repeat, path, steps in candidates:
        key = (task, seed_key_for(run, repeat))
        groups.setdefault(key, []).append((run, task, repeat, path, steps))

    chosen: list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]] = []
    for key, items in groups.items():
        items.sort(key=lambda it: (len(it[4]), -_run_recency_key(it[0].run_id)))
...
# ---------------------------------------------------------------------------
def build_records(
    rollouts: list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]],
) -> list[StepRecord]:
    out: list[StepRecord] = []
    for run, task, repeat, path, steps in rollouts:
        for step in steps:
            ...
            out.append(StepRecord(
                ...
                seed_key=seed_key_for(run, repeat),
                messages=messages,
                images=images,
...
Self-distilled Qwen3-VL-4B-Instruct SFT dataset for Phase 1 dummy training.
Built from successful UI-Voyager android_world rollouts under
[`MARS-Voyager/eval_results/`](../../../MARS-Voyager/eval_results/).

- Steps: **{n_steps}**, unique rollouts: **{n_rollouts}**, unique tasks: **{n_tasks}**
- Images mode: **{images_mode}**
- Built by [`training/data/build_sft_v0.py`](../build_sft_v0.py)
- Plan: [`tasks/training-codebase/sft-data/plan.md`](../../../tasks/training-codebase/sft-data/plan.md)
- Pins: [`training/PINS.md`](../../PINS.md)

## Files

- `train.parquet` — dataset (verl SFT layout: `messages` + `images` + Pattern-C source fields).
...
def main() -> int:
    ...
    ap.add_argument("--out-dir", type=Path,
                    default=Path("training/data/sft_v0"),
    ...
    funnel = FilterFunnel()
    survivors: list[tuple[RunInfo, str, int, Path, list[dict[str, Any]]]] = []
    system_prompts_seen: set[str] = set()
    canonical_system_prompt = ""

    for run in runs:
        for task, repeat, path in iter_succ_rollouts(run):
            ...
            survivors.append((run, task, repeat, path, steps))
    ...
    write_parquet(records, parquet_path, images_mode=args.images_mode)
